...
const Dashboard = () => {
    ...
    return (
        <main className='flex h-screen w-full overflow-hidden bg-black text-white'>
            {/* Sidebar */}
            <Sidebar chat={chat} chatList={chatList} currentChatId={currentChatId} openChat={openChat} user={user}/>
                    ...
                    <>
                        <div className='messages flex-1 space-y-6 overflow-y-auto px-6 py-6'>
                            {chats[currentChatId].messages.map((message, index) => (
                                <div
                                    key={index}
                                    className={`max-w-[95%] w-fit rounded-2xl px-15 py-2 text-sm leading-7 ${message.role === 'user'
